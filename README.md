...
Velocity profile means the motors will reach the given target while moving at a specified speed, 
...
you want to set the time to reach the target to `1/x + 0.1`s. 
...
A bulk message contains messages for every motor (no more than one per motor if sending), 
those motor messages are prepared by the `Motor` class in `multi_controller.py`. 
The `MotorHandler` then coordinates several `Motor` objects and publishes the resulting bulk message on the bus.
